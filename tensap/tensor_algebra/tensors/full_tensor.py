...
class FullTensor:
    ...
    def kron(self, tensor2):
        ...
        shape2 = np.concatenate(
            (tensor2.shape, np.ones(
                order_max - order2, dtype=int)))
    ...
    def orth(self, dim=None):
        ...
        dims = np.concatenate(
            (np.arange(dim), np.arange(
                dim + 1, tensor.order), [dim]))
    ...
    def outer_product_eval_diag(self, tensor2, dims1, dims2, diag=False):
        ...
        """
        return self.tensordot_eval_diag(
            tensor2, None, None, dims1, dims2, diag)
