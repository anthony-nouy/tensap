...
class FunctionalTensor(tensap.Function):
    ...
    def __init__(self, tensor=None, bases=None, fdims=None):
        ...
        if fdims is None:
            if tensor.order != len(bases):
                raise ValueError(
                    "Bases must contain as many bases as the "
            ...
            if len(fdims) != len(bases):
                raise ValueError(
                    "The number of functional dimensions must "
    ...
    def variance(self, *measure):
        ...
        *measure : tensap.RandomVector, optional
        ...
        if np.isscalar(mean):
            var = self.dot_product_expectation(
                self, None, *measure) - mean ** 2
        else:
            raise NotImplementedError("Method not implemented.")
        return var
        ...
        *measure : tensap.RandomVector, optional
        ...
        if fdims is None:
            if self.tensor.order == f_2.tensor.order:
                ...
                raise ValueError(
                    "Tensors u and v do not have the same "
    ...
    def norm(self, *measure):
        ...
        *measure : tensap.RandomVector, optional
        ...
        tensor = self.tensor.tensor_matrix_product(
            gram_matrix, range(self.tensor.order)
        )
    ...
    def __mul__(self, f_2):
        if isinstance(f_2, (FunctionalTensor, tensap.FunctionalTensor)):
            ...
            if (
                isinstance(out.tensor, tensap.TreeBasedTensor)
                and out.tensor.ranks[out.tensor.tree.root - 1] > 1
            ):
                if (
                    self.tensor.ranks[self.tensor.tree.root - 1]
    ...
    def parameter_gradient_eval(self, alpha, x=None, *args):
        ...
        if self.evaluated_bases:
            ...
            raise ValueError(
                "Must provide the evaluation points or the " + "bases evaluations."
            )
        ...
        if isinstance(self.tensor, tensap.TreeBasedTensor):
            ...
            if np.all(self.tensor.is_active_node):
                ...
                for pa in np.unique(
                    ...
                    if len(ind) > 1:
                        tensors[pa - 1] = self.tensor.tensors[
                            pa - 1
                        ].tensor_matrix_product_eval_diag(
                            [bases_eval[x] for x in dims_loc],
                ...
                keep_ind = tensap.fast_setdiff(
                    np.arange(tree.nb_nodes), tree.dim2ind[dims] - 1
                )
        ...
        if (
            isinstance(self.tensor, tensap.TreeBasedTensor)
            and not tree.is_leaf[alpha - 1]
        ):
    ...
    def eval_with_bases_evals(self, bases_eval, dims=None):
        ...
        if len(dims) == self.tensor.order:
            ...
            out.tensor = out.tensor.tensor_matrix_product_eval_diag(
                bases_eval, fdims_eval
            )
            fdims_eval.sort()
