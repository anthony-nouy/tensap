...
class MultiIndices:
    ...
    def weighted_norm(self, p, w, k=None):
        ...
        if p == np.inf:
            norm = np.max(
                self.array[k, :] * np.tile(np.ravel(w), (np.size(k), 1)),
                axis=1)
        else:
            norm = np.power(
                np.sum(
                    (self.array[k, :] * np.tile(np.ravel(w), (np.size(k),
                                                              1))) ** p, axis=1, ), 1 / p,
            )
        return norm
    ...
    def get_maximal_indices(self):
        ...
        neighbours = np.reshape(neighbours, [n * dim, dim],
                                order="F").astype(int)
        ok = np.any(np.all(neighbours == self.array[:, np.newaxis],
                           axis=2), axis=0)
    ...
    def get_margin(self):
        ...
        n = self.cardinal()
        neighbours = np.tile(
            np.transpose(np.expand_dims(self.array, 2),
                         [0, 2, 1]), [1, dim, 1]
        ) + np.tile(np.transpose(np.expand_dims(np.eye(dim),
                                                2), [2, 0, 1]), [n, 1, 1])
        neighbours = np.reshape(neighbours, [n * dim, dim],
                                order="F").astype(int)
    ...
    def get_reduced_margin(self):
        ...
        dim = self.ndim()
        neighbours = np.tile(
            np.transpose(np.expand_dims(I_marg.array, 2),
                         [0, 2, 1]), [1, dim, 1]
        ) - np.tile(
            np.transpose(np.expand_dims(np.eye(dim), 2),
                         [2, 0, 1]),
            [I_marg.cardinal(), 1, 1],
        )
