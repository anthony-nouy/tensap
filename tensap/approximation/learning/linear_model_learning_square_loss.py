...
class LinearModelLearningSquareLoss(tensap.LinearModelLearning):
    ...
    def _solve_ols(self):
        ...
        if self.linear_solver == "solve":
            ...
            try:
                ...
                sol = np.linalg.lstsq(B, np.matmul(
            ...
            try:
                ...
                sol = np.linalg.lstsq(r_A, np.matmul(
            ...
            if self.error_estimation_type == "residuals":
                ...
                if self.linear_solver == "solve":
                    ...
                    error, delta = self._compute_cv_error(
                        y, second_moment, A, sol, self.linear_solver, B_inv
                    )
                elif self.linear_solver == "qr":
                    error, delta = self._compute_cv_error(
                        y, second_moment, A, sol, self.linear_solver, q_A, r_A
                    )
            output["error"] = error
            output["delta"] = delta
    ...
    def _solve_regularized_ls(self):
        ...
        if self.regularization_type == "l0":
            ...
            solpath = linear_model.orthogonal_mp(
                A_omp,
                y,
                copy_X=True,
                n_nonzero_coefs=np.min((N, P)),
                return_path=True,
                **options_loc
            )
    ...
    def _select_optimal_path(self, solpath=None):
        ...
        for i in range(pattern.shape[1]):
            ...
            A_red = A[:, ind]
            if "gram_matrix" in self.error_estimation_options:
                self.error_estimation_options["gram_matrix"] = gram_matrix[ind, ind]
            ...
            if self.linear_solver == "qr":
                ...
                err[i], delta[:, i] = self._compute_cv_error(
                    y, second_moment, A_red, sol_red, "qr", q_A, r_A
                )
                ...
                err[i], delta[:, i] = self._compute_cv_error(
                    y, second_moment, A_red, sol_red, "solve", C_red
                )
        ...
        if self.error_estimation_type == "leave_out":
            ...
            for _, test in cvp:
                ...
                if n == 1:
                    ...
                    y_loc = y[test, :]
                delta_loc = np.linalg.solve(
                    np.eye(len(test)) - H[np.ix_(test, test)],
