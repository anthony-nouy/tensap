...
class TensorPrincipalComponentAnalysis:
    ...
    def alpha_principal_components(
            self, fun, shape, alpha, tol, B_alpha, I_alpha):
        """
        ...
        if tol < 1 and self.pca_adaptive_sampling:
            A = tensap.FullTensor(
                np.zeros((I_alpha.shape[0], 0)), 2, [I_alpha.shape[0], 0]
            )
            for k in range(N):
                ...
                if sin_val[-1, -1] < 1e-15 or pc.shape[1] < np.ceil(
                    (k + 1) / self.pca_sampling_factor
                ):
                    break
    ...
    def hopca(self, fun, shape):
        ...
        if np.ndim(self.max_rank) == 0 or len(self.max_rank) == 1:
            ...
            raise ValueError(
                "max_rank should be a scalar or an array of " + "length d."
            )
        ...
        for alpha in range(d):
            ...
            f_pc_alpha, output_alpha = solver.alpha_principal_components(
                fun, shape, alpha, tol_alpha, B_alpha, I_alpha
            )
    ...
    def tt_approximation(self, fun, shape):
        ...
        if np.ndim(self.max_rank) == 1 and len(self.max_rank) == d - 1:
            ...
            raise ValueError(
                "max_rank should be a scalar or an array of " + "length d-1."
            )

        return solver.tree_based_approximation(
            fun, shape, tree, is_active_node)
    ...
    def tree_based_approximation(self, fun, shape, tree, is_active_node=None):
        ...
        if np.ndim(self.tol) == 0 or len(self.tol) == 1:
            ...
            raise ValueError(
                "tol should be a scalar or an array of length " + "tree.nb_nodes."
            )
        ...
        if np.ndim(self.max_rank) == 0 or len(self.max_rank) == 1:
            ...
            raise ValueError(
                "max_rank should be a scalar or an array of " + "length tree.nb_nodes."
            )
        ...
        for level in np.arange(np.max(tree.level), 0, -1):
            for alpha in np.intersect1d(
                tree.nodes_with_level(level), tree.internal_nodes
            ):
                S_alpha = tree.children(alpha)
                B_alpha = TensorPrincipalComponentAnalysis._tensor_product_b_alpha(
                    alpha_basis[S_alpha - 1]
                )
                alpha_grids[alpha - 1] = tensap.FullTensorGrid(
                ...
                shape_alpha = np.concatenate(
                    (
                        [x.shape[1] for x in alpha_basis[S_alpha - 1]],
                        [pc_alpha.shape[1]],
                    )
                )
                tensors[alpha - 1] = tensap.FullTensor(
                    pc_alpha, len(S_alpha) + 1, shape_alpha
                )
