...
class TreeBasedTensorLearning(tensap.TensorLearning):
    ...
    def __init__(self, tree, is_active_node, *args):
        ...
        *args : tuple
        ...
        self.linear_model_learning_parameters["basis_adaptation_internal_nodes"] = False
    ...
    def initialize(self):
        ...
        if self.initialization_type == "random":
            f = tensap.TreeBasedTensor.randn(
                self.tree, self.rank, shape, self.is_active_node
            )
        elif self.initialization_type == "ones":
            f = tensap.TreeBasedTensor.ones(
                self.tree, self.rank, shape, self.is_active_node
            )
        elif self.initialization_type == "initial_guess":
            f = self.initial_guess
            if not np.all(f.ranks == self.rank):
                tr = tensap.Truncator(tolerance=np.finfo(
                    float).eps, max_rank=self.rank)
                f = tr.truncate(f)
        elif (
            ...
            if not np.all(self.rank == 1):
                raise NotImplementedError(
                    "Initialization only implemented " +
            ...
            if np.ndim(self.training_data[1]) == 2:
                shape = np.concatenate(
                    (np.full(nb_child, 1), [self.training_data[1].shape[1]])
                )
            else:
                shape = np.full(nb_child, 1)
            f.tensors[self.tree.root - 1] = tensap.FullTensor(
                np.mean(self.training_data[1]), shape=shape
            )
            f.update_attributes()
            ...
            if self.output_dimension != 1:
                print(
                    "Canonical initialization not implemented for "
                ...
                if not np.all(f.ranks == self.rank):
                    tr = tensap.Truncator(
                        tolerance=np.finfo(float).eps, max_rank=self.rank
                    )
                    f = tr.truncate(f)
        else:
        ...
        if not np.all(f.ranks == self.rank):
            f = TreeBasedTensorLearning.enriched_edges_to_ranks_random(
                f, self.rank)
    ...
    def randomize_exploration_strategy(self):
        ...
        for level in np.arange(np.max(self.tree.level), -1, -1):
            active_nodes = np.intersect1d(
                self.tree.nodes_with_level(
                    level), np.nonzero(self.is_active_node)[0]
            )
            _, ind = np.intersect1d(
                self._exploration_strategy, active_nodes, return_indices=True
            )
            strategy[ind] = self._exploration_strategy[np.random.permutation(
                ind)]
        return strategy
    ...
    def prepare_alternating_minimization_system(self, f, mu):
        ...
        if self.linear_model_learning[mu - 1].basis_adaptation:
            if np.isin(mu, tree.internal_nodes):
                if self.linear_model_learning_parameters[
                    ...
                    tr = tensap.Truncator(
                        tolerance=np.finfo(
                            float).eps, max_rank=np.max(f.tensor.ranks)
                    )
            ...
            self.linear_model_learning[
                mu - 1
            ].basis_adaptation_path = self.create_basis_adaptation_path(
                f.tensor.ranks, mu
            )
        else:
            f.tensor = f.tensor.orth_at_node(mu)
        ...
        if mu == tree.root:
            ...
            self.linear_model_learning[mu - 1].initial_guess = np.reshape(
                ...
                [-1, f.tensor.ranks[tree.root - 1]],
                order="F",
            )
        else:
            A = np.reshape(
                grad.data, [grad.shape[0], -1, f.tensor.ranks[tree.root - 1]], order="F"
            )
            self.linear_model_learning[mu - 1].initial_guess = np.reshape(
        ...
        if isinstance(self.loss_function, tensap.DensityL2LossFunction):
            if isinstance(self.training_data, list) and len(self.training_data) == 2:
                ...
                if tree.is_leaf[mu - 1]:
                    ...
                    ind = np.setdiff1d(
                        range(self.order), np.nonzero(tree.dim2ind == mu)[0]
                    )
                ...
                b = []
        elif isinstance(self.training_data, list) and len(self.training_data) == 2:
            b = self.training_data[1]
            self.linear_model_learning[mu -
                                       1].shared_coefficients = mu != tree.root

        return self, A, b, f

    def set_parameter(self, f, mu, coef):
        f.tensor.tensors[mu - 1] = tensap.FullTensor(
            coef, shape=f.tensor.tensors[mu - 1].shape
        )
    ...
    def canonical_initialization(self, rank):
        ...
        solver.alternating_minimization_parameters = deepcopy(
    ...
    def canonical_correction(self, f, rank):
        ...
        if isinstance(solver.training_data, list) and len(solver.training_data) == 2:
    ...
    def create_basis_adaptation_path(self, ranks, alpha):
        ...
        if tree.is_leaf[alpha - 1]:
            path_alpha = self.bases_adaptation_path[
                np.nonzero(tree.dim2ind == alpha)[0][0]
            ].astype(bool)
            ranks = ranks[alpha - 1]
            ...
            if np.all(self.is_active_node[ch - 1]):
                ...
                path = np.full(
                    np.prod(ranks[ch_a - 1]) * ranks[alpha - 1], True)
            else:
                path_alpha = []
                for nod in ch:
    ...
    def new_rank_selection(self, f):
        if self.rank_adaptation_options["rank_one_correction"]:
            ...
            s_local.rank = TreeBasedTensorLearning.make_ranks_admissible(
                f.tensor, f.tensor.ranks + ranks_add
            )[0]
        ...
        sin_val = np.array(
            [np.nan if x is None else x for x in sin_val], dtype=object)
        ...
        sin_val_min[
            [
                x / tensor_for_selection.norm() < np.finfo(float).eps
                if not np.isnan(x)
                else False
                for x in sin_val_min
            ]
        ] = np.nan
        ...
        rank = f.tensor.ranks
        desc = np.setdiff1d(
            np.arange(1, tree.nb_nodes + 1), np.nonzero(tree.is_leaf)[0] + 1
        )
        ...
        for level in np.arange(1, np.max(tree.level)):
            nod_lvl = np.intersect1d(
                tree.nodes_with_level(level), cannot_be_increased_nodes
            )
            for nod in nod_lvl:
                pa = tree.parent(nod)
        ...
        if np.all(np.isnan(sin_val_min)):
            ...
            enriched_nodes = (
                np.nonzero([not np.isnan(x) and x >= theta for x in sin_val_min])[
                    0] + 1
            )
        ...
        if not f.tensor.is_admissible_rank(new_rank):
            ...
            sin_val_min_sorted = sin_val_min_sorted[np.isfinite(
                sin_val_min_sorted)]

            for sv in sin_val_min_sorted:
                new_rank = np.array(rank_theta)
                ind = [x >= sv if not np.isnan(
                    x) else False for x in sin_val_min]
                new_rank[ind] += 1
                if f.tensor.is_admissible_rank(new_rank):
                    enriched_nodes = np.concatenate(
                        (enriched_nodes_theta, np.nonzero(ind)[0] + 1)
                    )
                    break
            if not f.tensor.is_admissible_rank(new_rank):
    ...
    def adapt_tree(self, f, cv_error, test_error, output, *args):
        ...
        if np.any(f.tensor.ranks[f.tensor.active_nodes - 1] == 0):
            print(
                "Some ranks equal to 0, disabling tree adaptation for " + "this step."
            )
            return self, f, output
        if self.tree_adaptation_options["tolerance"] is not None:
            ...
            print(
                "Must provide a tolerance for the tree adaptation in "
        ...
        for iteration in range(
            ...
            for alpha in alpha_list:
                if self.linear_model_learning[alpha - 1].basis_adaptation:
                    if np.isin(alpha, tree.internal_nodes):
                        if self.linear_model_learning_parameters[
                            ...
                            f.tensor = tr.hsvd(f.tensor)
                        elif np.all(f.tensor.is_active_node[tree.children(alpha) - 1]):
                            self.linear_model_learning[
                                alpha - 1
                    ...
                    if self.rank_adaptation_options["type"] == "dmrg":
                        self.linear_model_learning[
                            alpha - 1
                        ].basis_adaptation_path = self.create_basis_adaptation_path_dmrg(
                            f.tensor.ranks, alpha
                        )
                    elif self.rank_adaptation_options["type"] == "dmrg_low_rank":
                        self.linear_model_learning[
                            alpha - 1
                        ].basis_adaptation_path = self.create_basis_adaptation_path_dmrg_low_rank(
                            f.tensor.ranks, alpha
                        )
                    else:
                ...
                if isinstance(self.loss_function, tensap.DensityL2LossFunction):
                    if (
                        ...
                        if tree.is_leaf[alpha - 1]:
                            ...
                            ind = np.setdiff1d(
                                range(self.order), np.nonzero(
                                    tree.dim2ind == alpha)[0]
                            )
                            C = [x.data for x in f.tensor.tensors[tree.dim2ind - 1]]
                            b = a.tensor_vector_product(
                                [C[x] for x in ind], ind)
                            b = b.tensors[0].data
                        else:
                            b = f.tensor.dot(
                                y) / f.tensor.tensors[alpha - 1].data
                    else:
                        b = []
                elif (
                    isinstance(self.training_data, list)
                ...
                if self.rank_adaptation_options["type"] == "dmrg":
                    ...
                    self.linear_model_learning[alpha -
                                               1].training_data = [None, b]
                    ...
                    tr = tensap.Truncator()
                    tr.tolerance = self.tolerance["on_error"] / np.sqrt(
                        np.count_nonzero(f.tensor.is_active_node) - 1
                    )
                    ...
                    A = [np.reshape(x.data, [x.shape[0], -1], order="F")
                         for x in grad]

                    s_local = TreeBasedTensorLearning(
                        tensap.DimensionTree.trivial(2),
                        [True, True, False],
                        self.loss_function,
                    )
                    ...
                    s_local.rank_adaptation_options[
                        "max_iterations"
                    ] = self.rank_adaptation_options["max_rank"]
                    s_local.alternating_minimization_parameters = deepcopy(
                        self.alternating_minimization_parameters
                    )
                    s_local.alternating_minimization_parameters["display"] = False
                    ...
                    s_local.model_selection_options[
                        "type"
                    ...
                    a_gamma = C.tensor.tensors[0].transpose(
                        [1, 0]).reshape(sz_2)
                    ...
                    raise ValueError("Wrong rank adaptation type.")
                f.tensor.tensors[alpha - 1] = tensap.FullTensor(
                    a_alpha, np.size(sz_1), sz_1
                )
                f.tensor.tensors[gamma - 1] = tensap.FullTensor(
                    a_gamma, np.size(sz_2), sz_2
                )

            if self.rank_adaptation_options["post_alternating_minimization"]:
                print("\t\tPost alternating minimization.")
                s_local = TreeBasedTensorLearning(
                    self.tree, self.is_active_node, self.loss_function
                )
                ...
                s_local.tolerance["on_error"] = self.tolerance["on_error"] / np.sqrt(
                    np.count_nonzero(f.tensor.is_active_node) - 1
                )
                s_local.tolerance["on_stagnation"] = self.tolerance["on_stagnation"]
                s_local.alternating_minimization_parameters = deepcopy(
            ...
            if self.alternating_minimization_parameters["display"]:
                print(
                    "\tAlt. min. iteration %s/%i: stagnation = %2.5e"
                    % (
                        str(iteration).zfill(
                            len(
                                str(
                        ...
                        self.alternating_minimization_parameters["max_iterations"] - 1,
                        stagnation,
                    ),
                    end="",
                )
            ...
            if self.test_error and output["test_error"] < self.tolerance["on_error"]:
                output["flag"] = 1
                break

            if self.tree_adaptation and iteration > 0:
                C_old = f.tensor.storage()
                self, f, output = self.adapt_tree(
                    f, output["error"], None, output, iteration
                )
                adapted_tree = output["adapted_tree"]
                if adapted_tree:
                    if self.display:
                        print(
                            "\t\tStorage complexity before permutation "
                            + "= %i" % C_old
                        )
                        print(
                            "\t\tStorage complexity after permutation "
                            + "= %i" % f.tensor.storage()
                        )
                    if self.test_error:
                        ...
                        if self.display:
                            print(
                                "\t\tTest error after permutation "
                                + "= %2.5e"
                                % self.loss_function.test_error(
                                    f_eval_test, self.test_data
                                )
                            )
    ...
    def create_basis_adaptation_path_dmrg(self, ranks, alpha):
        ...
        for nod in np.setdiff1d(ch, alpha):
            ...
            if self.is_active_node[nod - 1]:
                p_gamma[tree.child_number(nod) - 1 + ind] = np.full(
                    (ranks[nod - 1], 1), True
                )
            else:
                rep = np.nonzero(nod == tree.dim2ind)[0][0]
                p_gamma[tree.child_number(nod) - 1 + ind] = self.bases_adaptation_path[
                    rep
                ].astype(bool)
        p_gamma[0] = p_alpha
    ...
    def enriched_edges_to_ranks_random(f, new_rank):
        ...
        for level in np.arange(1, np.max(tree.level) + 1):
            nod_lvl = np.intersect1d(
                tree.nodes_with_level(level), enriched_dims)
            for alpha in nod_lvl:
                gamma = tree.parent(alpha)
                rank = new_rank[alpha - 1] - f.ranks[alpha - 1]

                A = np.reshape(
                    f.tensors[alpha - 1].data, [-1, f.ranks[alpha - 1]], order="F"
                )
                A = np.hstack(
                    (
                        A,
                        np.tile(np.reshape(A[:, -1], [-1, 1]), [1, rank])
                        * (1 + np.random.randn(A.shape[0], rank)),
                    )
                )
                A[:, -1 - rank -
                    1:] /= np.sqrt(np.sum(A[:, -1 - rank - 1:] ** 2, 0))
    ...
    def make_ranks_admissible(f, rank):
        ...
        ind = [f.tree.is_leaf[x] and rank[x] > f.tensors[x].shape[0]
               for x in nodes - 1]
