...
class PolynomialFunctionalBasis(tensap.FunctionalBasis):
    ...
    def __eq__(self, basis_2):
        if not isinstance(basis_2, PolynomialFunctionalBasis):
            ...
            out = self.basis == basis_2.basis and np.array_equal(
                self.indices, basis_2.indices
            )
        return out
    ...
    def kron(self, q):
        ...
        def fun(x):
            return np.reshape(
                np.tile(np.expand_dims(self.eval(x), 2), (1, 1, q.cardinal()))
                * np.tile(
                    np.transpose(np.expand_dims(q.eval(x), 2), [0, 2, 1]),
                    (1, self.cardinal(), 1),
                ),
