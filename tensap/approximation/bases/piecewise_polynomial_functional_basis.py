...
class PiecewisePolynomialFunctionalBasis(tensap.FunctionalBasis):
    ...
    def eval(self, x, indices=None):
        ...
        for i in range(self.points.size - 1):
            ...
            if I.size > 0:
                ...
                scale = l[i]  # Scale factor
                hx[np.ix_(I, J)] = pu[I, :self.p[i] + 1] / np.sqrt(scale)
    ...
    def gauss_integration_rule(self, n):
        """
        Returns an Integration Rule that integrates exactly piecewise polynomials of degree 2*n-1, using n-points
    ...
    def interpolation_points(self):
        """
        Returns interpolation points for the PiecewisePolynomialFunctionalBasis.
        """
        unique_p = np.unique(self.p)
        u = [0.5 * (1 + tensap.chebyshev_points(p + 1)) for p in unique_p]  # Chebyshev points scaled
    ...
    def magic_points(self):
        ...
        for i in range(unique_p.size):
            ...
            pol = tensap.PolynomialFunctionalBasis(U.orthonormal_polynomials(),np.arange(unique_p[i]+1))
